...
def test_income_and_upkeep():
    s = _state(level=2, troops=2000, food=1000, gold=0)
    advance_turn(s, [])
...
def test_starvation_desertion():
    s = _state(level=1, troops=40000, food=0)
    advance_turn(s, [])
    c = s.cities["성도"]
    # ⭐굶주림 통일: 수입 500 < 주둔 병량 1000 → 못 먹인 인원 500×40=20000의 30%=6000 탈영(절벽 폐기)
    unfed = (40000 // GARRISON_TROOPS_PER_FOOD - CITY_INCOME_FOOD) * GARRISON_TROOPS_PER_FOOD
    assert c.food == 0 and c.troops == 40000 - round(unfed * STRANDED_DESERTION)
    assert any("[병량]" in h and "탈영" in h for h in s.history)


def test_size_zero_no_economy():
    s = _state(level=0, troops=5000, food=0, gold=7)
    advance_turn(s, [])
    c = s.cities["성도"]
...
def test_scenario_has_sizes():
    s = load_scenario()
    assert all(c.level >= 1 for c in s.cities.values())   # 실 시나리오는 전 도시 규모 큐레이트
...
def test_field_army_upkeep_charged_to_origin():
    ...
    s = _war_state(level=2, troops=10000, food=5000, gold=0)
...
def test_field_army_starvation_deserts_op():
    # 창고 고갈 시 탈영은 원정군에서(주둔은 먼저 먹음). 못 먹인 250×20=5000의 30%=1500 탈영(⭐통일 공식).
    s = _war_state(level=1, troops=20000, food=0, gold=0)
    advance_turn(s, [Battle(kind="전투", mode="공성", origin="성도", target="장안", troops=10000)])
    op = s.operations[0]
    assert op.committed_troops == 10000 - round(5000 * STRANDED_DESERTION)
    assert s.cities["성도"].food == 0
...
def test_stranded_op_starves():
    ...
    s = GameState(
        cities={"장안": City(name="장안", owner="위", troops=0, level=1)},
...
def test_food_runway_alert_and_surplus():
    ...
    s = _state(level=1, troops=40000, food=1200)   # 수입 500 − 주둔 1000 = −500/월 → 1200/500 = 2개월
    assert food_runway(s, "성도") == 2
    s2 = _state(level=1, troops=10000, food=0)     # 주둔 250 < 수입 500 → 흑자
    assert food_runway(s2, "성도") is None


def test_food_net_in_brief():
    # ⭐월 수지 상시 박기(사용자 2026-09-02): brief 도시 줄에 (월±n) — 소비 산수를 결론으로.
    from src.decide import brief
    from src.engine import food_net
    s = _state(level=1, troops=40000, food=1200)
    assert food_net(s, "성도") == -500
    assert "(월-500)" in brief(s, "촉")
    s2 = _state(level=1, troops=10000, food=0)
    assert food_net(s2, "성도") == +250
    assert "(월+250)" in brief(s2, "촉")
