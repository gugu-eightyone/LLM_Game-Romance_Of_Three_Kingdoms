...
- **⚠피침 표시가 붙은 우리 도시**는 공격받는 중이다. 대응 3가지: ① 접근 중인 적을 길에서 요격 = **그 도시에서
  적이 오는 방면(경보에 표시된 방면 도시)으로 야전 출격** ② 다른 도시에서 그 도시로 구원 야전(주둔) ③ 호송으로 증원.
  공성으로는 지원할 수 없다.
- **수성은 자동이다** — 주둔 병력은 명령 없이도 성벽 보너스를 받으며 그 도시를 지킨다. 같은 도시를 출발지이자
  목표로 하는 출격(제자리 야전)은 무의미하며 기각된다. 성을 나가 싸우고 싶으면 적이 오는 **방면**을 목표로 하라.
