...
- **⚠피침 표시가 붙은 우리 도시**는 공격받는 중이다. **다른 도시에서** 그 도시 방면 야전(구원/주둔) 또는
  호송으로 지원하라. 공성으로는 지원할 수 없다.
- **수성은 자동이다** — 도시의 주둔 병력은 명령 없이도 그 도시를 지킨다. 같은 도시를 출발지이자 목표로 하는
  출격(제자리 야전)은 무의미하며 기각된다.
    ...
    if own:  # 피침 경보: "내 도시가 공격받는 중"을 추론시키지 않고 결론으로 박음(전부 결정론)
        for t in state.operations:
            if t.faction != c.owner and getattr(t.action, "target", None) == name \
                ...
                eta = "교전 중" if t.stage == "교전" else f"약 {max(0.0, t.threshold - t.progress):g}개월 후 도착"
